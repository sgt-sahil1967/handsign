...
# Global variables for model initialization (loaded once)
hands = None
...
def predict_hand_sign(image):
    """
    Predict hand sign from an image frame.
    This function uses the EXISTING prediction logic from the original app.py
    
    Args:
        image: numpy array (BGR format from OpenCV)
    
    Returns:
        dict with 'hand_sign', 'confidence', 'finger_gesture' or None if no hand detected
    """
    global point_history, finger_gesture_history, hands, keypoint_classifier
    global keypoint_classifier_labels, point_history_classifier, point_history_classifier_labels
    ...
    if results.multi_hand_landmarks is not None:
        ...
        confidence = hand_sign_confidence
...
@app.route('/')
def login():
    """Login page - no real authentication"""
    return render_template('login.html')


@app.route('/dashboard')
def dashboard():
    """Dashboard page showing system status and model accuracy"""
    return render_template('dashboard.html')


@app.route('/detect')
def detect():
    """Hand sign detection page with webcam"""
...
def predict():
    """
    API endpoint for hand sign prediction.
    ...
    try:
        ...
        # Flip image horizontally (mirror effect like original app)
        image = cv.flip(image, 1)
        
        # Get prediction using EXISTING prediction logic
        result = predict_hand_sign(image)
        
        if result:
            return jsonify({
                'success': True,
                'hand_sign': result['hand_sign'],
                'confidence': result['confidence'],
                'finger_gesture': result['finger_gesture'],
